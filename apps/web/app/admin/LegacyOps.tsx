...
function GoonOpsPanel({
  ...
  const runPreview = async (category: string) => {
    if (!p) return;
    try {
      const d = await cc<{ line?: string; name?: string }>("/api/admin/goons/preview", {
        body: { handle: p.handle, category },
      });
...
function RugBansPanel({
  ...
  const load = useCallback(() => {
    cc<{ selfServeUnban: boolean; rugBanHours: number[]; banned: BannedRow[] }>(
      "/api/admin/banned",
    )
      .then(setData)
      .catch(() => {});
...
function BetaList() {
  ...
  const load = useCallback(() => {
    cc<BetaData>("/api/admin/beta").then(setData).catch(() => {});
  }, []);
  useEffect(load, [load]);
  ...
  const importText = async (text: string) => {
    ...
    setBusy(true);
    try {
      const r = await cc<{ added: number; already: number; invalid: number; total: number }>(
...
function FeedbackList() {
  ...
  useEffect(() => {
    cc<Feedback[]>("/api/admin/feedback")
      .then(setItems)
      .catch(() => {});
    const t = setInterval(
      () => void cc<Feedback[]>("/api/admin/feedback").then(setItems).catch(() => {}),
      15000,
    );
    return () => clearInterval(t);
  }, []);
...
function PitBossPanel() {
  ...
  const load = useCallback(() => {
    cc<TgAdminStatus>("/api/admin/telegram/status")
      .then(setStatus)
      .catch(() => {});
  }, []);
  ...
  const broadcast = async (kind: "announce" | "patch") => {
    ...
    setMsg("");
    try {
      await cc("/api/admin/telegram/broadcast", { body: { text, kind } });
  ...
  const setup = async (force: boolean) => {
    ...
    setMsg("");
    try {
      const r = await cc<{ posted: number; alreadyDone: boolean }>("/api/admin/telegram/setup", {
        body: { force },
      });
...
/**
 * The established live-ops console, mounted inside the Command Center.
 *
 * It no longer holds an admin key of its own or gates on a dev wallet — the
 * Command Center shell owns identity, and every call below goes through the
 * Command Center client, so a staff session authenticates them.
 */
export function LegacyOps() {
  ...
  const load = useCallback(async () => {
    try {
      setError("");
      setOverview(await cc<Overview>("/api/admin/overview"));
  ...
  const act = async (path: string, body?: unknown, method = "POST") => {
    try {
      setError("");
      await cc(path, { body: body ?? {}, method });
      void load();
    } catch (e) {
      setError((e as Error).message);
    }
    ...
    <div className="space-y-6">
      ...
      {overview && (
        <section>
          ...
          <BurgerOpsPanel settings={overview.settings.burger} act={act} />
        </section>
      )}
      ...
      {overview && (
        <section>
          ...
          <GoonOpsPanel settings={overview.settings.goons} act={act} />
        </section>
      )}
